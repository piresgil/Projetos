...
public class Batalha extends StackPane {
    ...
    public Batalha(Runnable onFinish, List<Personagem> herois, List<Personagem> bestas) {
    ...
    public void initialize() {
        logView.refresh();
        // System.out.println("Inicialização das listas: Heróis: " + getHerois() + ", Bestas: " + getBestas());
        ...
        // Inicializa logBatalha
        logBatalha = FXCollections.observableArrayList();
        logView.setItems(logBatalha);  // Certifique-se de que a ListView está conectada à ObservableList

        System.out.println(logView.getItems());
        System.out.println(logBatalha.toString());
        ...
        // Carrega o CSS aqui em vez de no iniciarBatalha()
    ...
    private void executarBatalha(List<Personagem> herois, List<Personagem> bestas) {
        ...
        while (!herois.isEmpty() && !bestas.isEmpty()) {
            ...
            try {
                ...
                Thread.sleep(100); // depois pode dormir, se quiseres pausa visual
        ...
        Platform.runLater(() -> {
            finalizarBatalha(); // <--- Chamada aqui
            if (onFinish != null) {
                onFinish.run(); // <--- Executa o callback que foi passado pelo menu
            }
        });
    }
    ...
    private void processarTurno(int turno) {
        ...
        // Criar listas temporárias para as personagens que permanecerão vivas após o turno
        ...
        for (int i = 0; i < Math.min(herois.size(), bestas.size()); i++) {
            ...
            // Verifica se a besta ainda está viva após o ataque do herói
            int danoBesta;

            if (besta.getVida() > 0) { // Se a besta sobreviveu, ela contra-ataca
                danoBesta = besta.combater(heroi);
                //  adicionarLinhaLog(logBatalha, heroi.getNome() + " causa " + danoHeroi + " de dano a " + besta.getNome());
            ...
            // System.out.println("Turno For\n" + turno);
            // adicionarLinhaLog(logBatalha, "\nTURNO: for dentro processar turno" + i + "\n");

            if (heroi.getVida() > 0) {
                // adicionarLinhaLog(logBatalha, besta.getNome() + " contra-ataca com " + danoBesta + " de dano a " + heroi.getNome());
                adicionarLinhaLog(logBatalha, String.format("                %s atacou com %d causando %d de dano a %s.",
    ...
    private void finalizarBatalha() {
        ...
        String vencedor = herois.isEmpty() ? "As Bestas Venceram!" : "Os Heróis Venceram!";
        ...
        if (scene != null) {
            // Aqui você passa o battleRoot (que é um Node)
            Node owner = root;

            Platform.runLater(() -> {
                PopupUtils.showSuccessPopup("Batalha Terminada!", "Vencedores: " + vencedor, owner);
            });
    ...
    public void adicionarLinhaLog(ObservableList<TextFlow> list, String texto) {
        ...
        Platform.runLater(() -> {
            ...
            //System.out.println("Adicionando linha ao log: " + texto);  // Verifica se o log está sendo adicionado
    ...
    private void atualizarPalavrasLog(TextFlow linha, String texto) {
        ...
        Map<String, String> palavrasEstilizadas = Map.of(
                "BUFF", "battle-log-buff-word",
                "VENCERAM", "battle-log-vencedor-word",
                "DANO", "battle-log-dano-word",
                "VIDA", "battle-log-dano-word",
                "DERROTADO", "battle-log-derrotado-word",
                "ARMADURA", "battle-log-armadura-word",
                "SOFREM", "battle-log-sofreu-word",
                "GANHAM", "battle-log-ganhou-word"
        );
