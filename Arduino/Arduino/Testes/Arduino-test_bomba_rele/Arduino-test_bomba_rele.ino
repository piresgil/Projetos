...
#define AUTHOR_EMAIL "teuemail@dominio.com"
#define AUTHOR_PASSWORD "senha_do_email"
#define RECIPIENT_EMAIL "destinatario@dominio.com"
...
void enviarEmail(String assunto, String corpo) {
  ...
  message.addRecipient("Usuário", RECIPIENT_EMAIL);
  message.text.content = corpo;

  SMTP_Config config;
  config.server.host = SMTP_HOST;
  ...
  if (!smtp.connect(&config)) {
    Serial.println("Erro a conectar ao servidor SMTP");
    return;
  }
...
void loop() {
  ...
  if (client) {
    ...
    for (int i = 0; i < maxRegistos; i++) {
      ...
      html += "<td>" + (historicoBomba[pos] ? "Ligada" : "Desligada") + "</td>";
      html += "</tr>";
    }
